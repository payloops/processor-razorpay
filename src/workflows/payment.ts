...
import type * as razorpayActivities from '../activities';
import type * as backendActivities from '../activities/backend-types';
import type { PaymentResult } from '@payloops/processor-core';

// Proxy to local Razorpay activities (runs on razorpay-payments queue)
const razorpay = proxyActivities<typeof razorpayActivities>({
...
export async function PaymentWorkflow(input: PaymentWorkflowInput): Promise<PaymentResult> {
  ...
  // Handle external payment completion (UPI, netbanking, etc.)
  ...
  try {
    ...
    const config = await backend.getProcessorConfig({ merchantId: input.merchantId, processor: 'razorpay' });
    ...
    // Step 2: Process payment via Razorpay (local activity)
    const result = await razorpay.processPayment({
      orderId: input.orderId,
    ...
    // Razorpay always returns requires_action for client-side checkout
